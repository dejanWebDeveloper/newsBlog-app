...
@section('content')
    <div class="container-fluid px-4">
        <h1 class="mt-4">@lang('Edit Article')</h1>
        ...
        <form id="store-article" enctype="multipart/form-data" action="{{route('admin.article.update-article', ['article'=>$article])}}" method="post">
            @csrf
            <div class="col-12">
                <div class="col-6 ">
                    ...
                    <input name="heading" type="text" class="form-control @error('heading') is-invalid @enderror"
                           placeholder="Heading of article" value="{{old('heading', $article->heading)}}">
                    <div>
                        @error('heading')
                ...
                <div class="col-6 mb-3">
                    ...
                    <input name="preheading" type="text" class="form-control @error('preheading') is-invalid @enderror"
                           placeholder="Preheading of article" value="{{old('preheading', $article->preheading)}}">
                    <div>
                        @error('preheading')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-6 mb-3">
                    ...
                    <img id="old-photo" style="width: 400px; height: 400px; padding-top: 10px" src="{{$article->getImageUrl()}}">
                ...
                <div class="col-6 mb-3">
                    ...
                    <select class="form-control @error('tags') is-invalid @enderror" name="tags[]" id="select_tags" multiple="multiple">
                        @foreach($tags as $tag)
                            <option @if(in_array($tag->id, old('tags', $article->tag()->pluck('tag_id')->toArray()))) selected
                ...
                <div class="col-6 mb-3">
                    ...
                    <textarea type="text" class="form-control @error('text') is-invalid @enderror"
                              name="text" id="article_text">{{old('text', $article->text)}}</textarea>
                    <div>
                        @error('text')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
        ...
        <script>
            document.getElementById('photo-input').addEventListener('change', function(event) {
                ...
                if (input.files && input.files[0]) {
                    // Sakrij staru sliku ako postoji
